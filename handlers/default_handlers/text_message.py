...
from utils.API_commands import (airport_information, flight_information, distance_time_information,
...
def ask_airport(message: Message):
    if len(message.text) == 3 and message.text.isalpha():
        inform = airport_information(message)
...
#     bot.send_message(message.chat.id, "Введите конец диапазона поиска (в формате:  ГГГГ-ММ-ДДTЧЧ:мм), например: "
#                                       "2023-04-04T20:00) Должен быть больше начала диапазона поиска, но не более чем "
#                                       "на 12 часов:")
...
def ask_flight_schedule(message: Message):
    ...
    if (inform_flight_schedule == 'Произошла ошибка при получении данных.' or
            inform_flight_schedule == 'Расписание рейсов не найдено.'):
...
def ask_airport_from(message: Message):
    if len(message.text) == 3 and message.text.isalpha():
        with bot.retrieve_data(message.chat.id, message.from_user.id) as data:
            ...
            bot.send_message(message.chat.id, "Введите код аэропорта назначения (3-значный IATA-код аэропорта."
...
def ask_airport_to(message: Message):
    ...
    bot.send_message(message.chat.id, f"{inform_distance_time}")
    bot.delete_state(message.from_user.id, message.chat.id)
...
def bot_echo(message: Message):
    if (("привет" or "приветствую" or "здравствуйте" or "добрый день" or "доброе утро" or "добрый вечер")
